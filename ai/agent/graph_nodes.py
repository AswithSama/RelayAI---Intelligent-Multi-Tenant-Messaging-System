...
def load_conversation_history_node(state: AgentState) -> Dict[str, Any]:
    """
    Loads customer/company template values and conversation history before Layer 1.

    conversation_history:
    - Includes recent messages up to and including the latest outbound/company message.
    - Does not include inbound customer messages after the latest outbound.
    - Those latest inbound messages are grouped earlier and passed as state["customer_message"].

    This avoids duplicating the current customer turn in both conversation_history
